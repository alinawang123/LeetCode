...
public class _322_CoinChange {
  private static final Logger logger = Logger.getLogger(_322_CoinChange.class.toString());
  ...
  class Solution {
    public int coinChange(int[] coins, int amount) {
      ...
      Arrays.fill(dp, amount+1); //autofill max amount
      dp[0] = 0;
      for(int c: coins) {
        for (int i = 1; i <= amount; i++) {
          if(i-c>=0){ //only when i-c>=0
            dp[i] = Math.min(dp[i], dp[i-c] +1);
          }
        }
      }
      return dp[amount] == amount+1? -1: dp[amount]; //check if it has been updated
    }
  }
